...
export function createLaneTextDeliverer(params: CreateLaneTextDelivererParams) {
  ...
  const streamText = async (
    ...
    const previewRevisionBeforeFlush = stream.previewRevision?.() ?? 0;
    ...
    if (typeof messageId !== "number") {
      if (!isFinal && isDraftPreviewLane(lane)) {
        if ((stream.previewRevision?.() ?? 0) > previewRevisionBeforeFlush) {
      ...
      if (
        ...
        !buttons &&
        typeof stream.materialize === "function"
      ) {
        ...
        if (typeof materializedMessageId === "number") {
          ...
          return result("preview-finalized", { content: text, messageId: materializedMessageId });
        }
        params.log(
